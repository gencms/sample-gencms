...
class Ion_auth
{
     ...
     * @author Mathew
     **/
    public function registration($username, $password, $email, $additional_data = array(), $group) //need to test email activation
    {
        $this->ion_auth_model->trigger_events('pre_account_creation');
        ...
        if (!$email_activation)
        {
            $id = $this->ion_auth_model->registration($username, $password, $email, $additional_data, $group);
            if ($id !== FALSE)
            {
                $this->set_message('account_creation_successful');
                $this->ion_auth_model->trigger_events(array('post_account_creation', 'post_account_creation_successful'));
