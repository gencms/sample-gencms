...
class Auth extends MY_Controller {
	...
	function index()
	{
		...
		else
		{
			...
			$this->main_template_view('dashboard', $data);
		}
	}
	...
	/**
	 * This method is responsible for listing all users
	 * @view   	  : list.php
	 */
	function list_users() {				
		...
		$config = array();
		$config["base_url"] = site_url() . "/user/list_users";
		$config["total_rows"] = $this->ion_auth->record_count("user");
		$config["per_page"] = 10;
		...
		$data["items"] = $this->ion_auth->get("user", NULL, "id", "id",$config["per_page"], $page);
					
		$data["section_title"] = "Users";	
	...
	/**
	* This method is responsble for editing a user
	* Loads the user_edit View
	* 
	**/
	...
	{  		
        ...
        if(!$this->form_validation->run()) {
			#validation failed
		    	...
		    	$group_id = 1;//$this->input->post("group_id");                          
		    	$additional_data = array("username" => $username);

		    	$user_insert_id = $this->ion_auth->registration($email, $password, $email, $additional_data, $group_id);	    	
		...
		if(!empty($id)){
			...
			$data['user'] = $this->ion_auth->get_new_user();	
		}
	...
	function login()
	{
		...
		else
		{
			...
			$this->main_template_view('/auth/login', $data);
	...
	function change_password()
	{
		...
		if ($this->form_validation->run() == false)
		{
			...
			//render
			$this->main_template_view('/auth/change_password', $data);
		}
		else
		{
	...
	function forgot_password()
	{
		...
		else
		{
            ...
            if ($row->count == 0){
                ...
                $this->ion_auth->set_message('forgot_password_captcha_wrong');
                $this->session->set_flashdata('message', $this->ion_auth->messages());
          //      return FALSE;
    ...
    function registration_user()
    {
        ...
        //validate form input
        $this->form_validation->set_rules('email', $this->lang->line('create_user_validation_email_label'), 'required|valid_email|is_unique[user.email]');
        $this->form_validation->set_rules('password', $this->lang->line('create_user_validation_password_label'), 'required|min_length[' . $this->config->item('min_password_length', 'ion_auth') . ']|max_length[' . $this->config->item('max_password_length', 'ion_auth') . ']|matches[password_confirm]');
        $this->form_validation->set_rules('password_confirm', $this->lang->line('create_user_validation_password_confirm_label'), 'required');
        ...
        if ($this->form_validation->run() == true && $this->ion_auth->registration($username, $password, $email, $additional_data, $groups))
        {
            //check to see if we are creating the user
            //redirect them back to the admin page
            $this->session->set_flashdata('message', $this->ion_auth->messages());
       ...
       //     $groups = $this->ion_auth->groups()->result_array();
            $groups = $this->ion_auth->grouplist();
            $data['groups'] = $groups;
            ...
            $view = 'registration';
            $this->_render_page_view($view, $data);
        }
        }
